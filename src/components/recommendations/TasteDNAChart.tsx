...
export function TasteDNAChart({
  ...
  const calculateDNA = (): Dimension[] => {
    ...
    themes.forEach((t) => {
      if (t === "slow-burn" || t === "strategist" || t === "twists") psychScore += 20;
      if (t === "fast" || t === "underdog" || t === "anti-hero" || t === "hype") actionScore += 20;
      if (t === "cozy" || t === "relatable") romanceScore += 20;
      if (t === "cyberpunk" || t === "fantasy" || t === "episodic") fantasyScore += 20;
      if (t === "character-drama" || t === "military" || t === "tears") dramaScore += 20;
    });
    ...
    <div className={`glass rounded-[28px] p-8 border border-white/10 relative overflow-hidden shadow-2xl ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="px-3 py-1 rounded-full glass border border-white/10 text-[10px] font-label uppercase tracking-widest text-warm-white/60 flex items-center gap-1.5">
          ...
          {hasProfile ? "Questionnaire Built" : "Awaiting Profile"}
        </div>
      </div>
